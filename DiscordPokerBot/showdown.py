...
from utils import send_board_images
...
async def finish_hand(ctx, t):
   ...
   if ok:
       ...
       for p in t.players:
           ...
           try:
               for card in p.hole:
                   from utils import code_to_url, card_code
                   await member.send(embed=ctx.bot.Embed().set_image(url=code_to_url(card_code(*card))))  # This line won't work with ctx.bot.Embed
           except Exception:
               pass  # we'll DM via pokerbot; safer approach below


       # Safer DM that doesn't rely on ctx.bot.Embed hack:
       for p in t.players:
           member = ctx.guild.get_member(p.user_id)
           try:
               for card in p.hole:
                   from utils import code_to_url, card_code
                   await member.send(embed=__import__("discord").Embed().set_image(url=code_to_url(card_code(*card))))
           except Exception:
