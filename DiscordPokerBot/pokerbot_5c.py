...
from showdown import handle_allin_runout, begin_showdown
...
async def start(ctx, sb: int, bb: int, min_buyin: int, max_buyin: int):
    """Create a heads-up table with blinds and buy-in limits."""
    if get_table(ctx):
...
async def begin(ctx):
    """Start a new hand (manual). The bot also auto-starts the next hand after each showdown."""
    t = get_table(ctx)
    if not t:
        return await ctx.reply("No table.")
    ok, msg = t.begin_hand()
    ...
    # DM each player's hole cards as images
    for p in t.players:
...
async def maybe_next_street(ctx, t: PokerTable):
    ...
    # if hand still running, show buttons and set 30s auto-fold
    if t.street not in ["idle", "showdown"]:
        current_player = t.players[t.turn_idx]
        await ctx.send(
            f"{current_player.name}, it’s your turn!",
            view=ActionView(bot, t, ctx)
        )

        async def auto_fold(player_id=current_player.user_id, name=current_player.name):
            await asyncio.sleep(30)
            # still same player’s turn?
            if (
                t.street not in ["idle", "showdown"]
                and t.players[t.turn_idx].user_id == player_id
            ):
                t.players[t.turn_idx].folded = True
                ...
                if len(alive) == 1:
                    ...
                    t.pot = 0
                    t.street = "idle"
                    await ctx.send(f"⏱ {name} timed out (auto-fold). {winner.name} wins the pot!")
                else:
                    t.turn_idx = (t.turn_idx + 1) % len(t.players)
                    await ctx.send(f"⏱ {name} timed out (auto-fold).")
                    await maybe_next_street(ctx, t)

        bot.loop.create_task(auto_fold())
...
async def allin(ctx):
    ...
    p.committed += pay
    t.pot += pay
    # keep current_bet at highest committed among live players
...
- `!poker buyin <amount>` → Buy in with chips (within min/max)
- `!poker begin` → Start a new hand (also auto-continues after each hand)
- `!poker status` → Show the current table state
- `!poker end` → End the table

**Actions (during your turn)**
- `!poker check` → Check (if no bet to call)
- `!poker call` → Call the current bet
- `!poker raise <amount>` → Raise by a specific amount
- `!poker allin` → Push all your chips in
...
if __name__ == "__main__":
    print("Poker bot (HU) with buttons + auto-fold timeout online.")
    bot.run(TOKEN)
